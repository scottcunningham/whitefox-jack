...
/* Demo: display all leds */
const uint8_t all_dark[83] = {
  0x24,
      ...
      case LED_MSG_ALL_TOGGLE:
        ...
        if(temp) {
          ...
          for(uint8_t i=0; i<9; i++) {
            is31_write_data(0,(uint8_t *)(all_dark+(9*i)),9);
            is31_write_data(1,(uint8_t *)(all_dark+(9*i)),9);
            chThdSleepMilliseconds(5);
          }
        } else {
          // turn on on pages 1 and 2
          for(uint8_t i=0; i<9; i++) {
            is31_write_data(0,(uint8_t *)(all_bright+(9*i)),9);
            is31_write_data(1,(uint8_t *)(all_bright+(9*i)),9);
            chThdSleepMilliseconds(5);
          }
        }
        break;
...
void hook_early_init(void) {
  ...
  /* Write "all_bright" (all LEDS on) into page 2 */
  for(i=0; i<9; i++) {
    is31_write_data(1,(uint8_t *)(all_dark+(9*i)),9);
    chThdSleepMilliseconds(5);
  }

  /* Write "all_bright" (all LEDS on) into page 1 -- default */
  for(i=0; i<9; i++) {
    is31_write_data(0,(uint8_t *)(all_bright+(9*i)),9);
    chThdSleepMilliseconds(5);
  }
